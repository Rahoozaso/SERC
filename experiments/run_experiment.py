...
import sys
import logging
import json # JSON 로딩 위해 추가 (data_loader에서 사용될 수 있음)
import pandas as pd
from typing import Dict, Any, List, Optional

# --- 프로젝트 루트 경로 설정 ---
# 이 스크립트 파일(run_experiment.py)의 상위 디렉토리(experiments)의 상위 디렉토리(프로젝트 루트)
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
# src 폴더를 Python 경로에 추가하여 src 내부 모듈 임포트 가능하게 함
sys.path.append(PROJECT_ROOT)

# --- src 폴더 내 모듈 임포트 ---
# (serc_framework 대신 src 사용, 파일명 정확히 명시)
...
def run_single_item(item: Dict[str, Any], method: str, model_name: str, config: Dict[str, Any],
    ...
    try:
        if method == 'serc':
            ...
            logging.info("CoVe 방법론 실행 (플레이스홀더)...")
            # cove_output = run_cove(query=query, model_name=model_name, config=config) # src.baselines.cove 임포트 필요
            cove_output = "CoVe 결과 플레이스홀더" # 임시
            result_data['cove_result'] = {'final_output': cove_output}
            result_data['final_output'] = cove_output

        elif method == 'self_refine':
            logging.info("Self-Refine 방법론 실행 (플레이스홀더)...")
            # self_refine_output = run_self_refine(query=query, model_name=model_name, config=config) # src.baselines.self_refine 임포트 필요
            self_refine_output = "Self-Refine 결과 플레이스홀더" # 임시
            result_data['self_refine_result'] = {'final_output': self_refine_output}
            result_data['final_output'] = self_refine_output
...
def main():
    ...
    parser.add_argument("--method", type=str, required=True,
                        choices=['serc', 'baseline', 'cove', 'self_refine'],
                        help="Method to run (serc, baseline, cove, self_refine).")
    ...
    # config 파일에 지정된 경로는 프로젝트 루트 기준 상대 경로로 가정
    ...
    if not os.path.exists(dataset_path):
        ...
        return
    try:
        # ⭐ src.data_loader의 load_dataset 함수 사용 (이 부분이 핵심) ⭐
