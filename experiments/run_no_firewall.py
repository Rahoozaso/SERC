...
def _detect_syndromes_batch(sentence_batches: List[Dict], 
    ...
    """
    [Atomic Retrieval Mode] Fact 단위 개별 검색
    """
    clean_facts = []
    syndromes_buffer = [] 
    facts_to_delete = []
        ...
        for block in correction_blocks:
            ...
            if orig_match and fixed_match:
                ...
                if matches:
                    ...
                    logging.info(f"🔗 Matched: '{true_key[:15]}...' -> Fixed")

    return fact_correction_map

# =============================================================================
# Main SERC Implementation (Firewall REMOVED)
# =============================================================================
...
def SERC(query: str, model_name: str, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Refusal Check (Cold Start)
    is_refusal = (len(baseline) < 50 or "sorry" in baseline.lower())
    if is_refusal:
        logging.warning(" Baseline refused. Attempting RAG-First Cold Start...")
        ...
        if rag_context and len(rag_context) > 10:
            ...
            return {"query": query, "final_output": "No info found.", "status": "no_info"}
    ...
    logging.info(f"Main Subject for Fact Extraction: {main_subject}")
    ...
    # Step 3: Detection (Atomic Retrieval)
