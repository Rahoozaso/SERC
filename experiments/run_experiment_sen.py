...
def prompt_judge_entity_consistency(desc_a: str, desc_b: str, model_name: str, config: dict) -> bool:
    ...
    match = re.search(r"\[judgment\]\s*(\[yes\]|yes|\[no\]|no)", cleaned)
...
def main():
    ...
    output_filename = f"serc_fact_in_sentence_t{T_MAX_TO_RUN}_rag{limit_str}{suffix_str}_{timestamp}.jsonl" 
    output_path = os.path.join(results_dir, output_filename)
    logging.info(f"결과는 다음 경로에 저장됩니다: {output_path}")

    results = []
    from tqdm import tqdm
    for item in tqdm(data, desc=f"SERC (Fact-in-Sentence, T={T_MAX_TO_RUN}, RAG)"): 
